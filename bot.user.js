...
(function(g, q) {
    ...
    function getAllFood() {
        ...
        elementList = getListmasedOnFunction(function(element) {
            ...
            for (var i = 0; i < m.length; i++) {
                if (!isMe && !v[element].isVirus && (v[element].size * 1.25 <= m[i].size) || (v[element].size <= 11)) {
                    return true;
                } else {
                    return false;
                }
    ...
    function findDestination() {
        ...
        if (m[0] != null) {
            ...
            for (var i = 0; i < allPossibleThreats.length; i++) {
                ...
                threatLineLeft = [
                    [offsetLeftX, offsetLeftY],
                    [offsetX, offsetY]
                ];
                threatLineRight = [
                    [offsetRightX, offsetRightY],
                    [offsetX, offsetY]
                ];
                ...
                if (null == x) {
                    for (a.font = '20px Ubuntu', b = 0; b < w.length; ++b) {
                        c = w[b].name || 'An unnamed cell',
                            Y || (c = 'An unnamed cell'), -1 != B.indexOf(w[b].id) ? (m[0].name && (c = m[0].name), a.fillStyle = '#FFAAAA') : a.fillStyle = '#FFFFFF',
                            c = b + 1 + '. ' + c,
                            a.fillText(c, 100 - a.measureText(c).width / 2, 70 + 24 * b);
                    }
                } else
                    for (b = c = 0; b < x.length; ++b) angEnd = c + x[b] * Math.PI * 2,
                        a.fillStyle = Ga[b + 1],
                        a.beginPath(),
                        a.moveTo(100, 140),
